...
import { SYMBOL_MAP, ASSET_CLASS_LABEL } from '@/lib/symbols';
...
export function StrategiesPage() {
  ...
  // Group signals by strategy for the engine columns.
  const byStrategy = useMemo(() => {
    const map: Record<StrategyId, Signal[]> = { trend: [], breakout: [], meanrev: [] };
  ...
  return (
    <div className="fade-in space-y-6">
      <SectionTitle
        title="Strategies"
        subtitle="Three engines scan every symbol in the watchlist. Real math — not curated numbers."
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        {ENGINE_IDS.map((id) => <EngineColumn key={id} id={id} signals={byStrategy[id]} riskStatus={risk.status} newsEvents={newsEvents} />)}
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-terminal-700/40">
                {shown.map((s) => <RankedRow key={`${s.symbol}-${s.strategy}`} s={s} riskStatus={risk.status} newsEvents={newsEvents} />)}
              </tbody>
            </table>
          </div>
        )}
...
function EngineColumn({ id, signals, riskStatus, newsEvents }: { id: StrategyId; signals: Signal[]; riskStatus: RiskStatus; newsEvents: { title: string }[] }) {
  ...
  return (
    <div className="card flex flex-col">
      ...
      <div className="divide-y divide-terminal-700/40 max-h-[28rem] overflow-y-auto">
        {signals.map((s) => <SignalRow key={s.symbol} s={s} riskStatus={riskStatus} newsEvents={newsEvents} />)}
      </div>
    </div>
  );
}

function SignalRow({ s, riskStatus, newsEvents }: { s: Signal; riskStatus: RiskStatus; newsEvents: { title: string }[] }) {
  const d = decimalsFor(s.symbol);
  const meta = SYMBOL_MAP[s.symbol];
  const active = s.direction !== 'NO TRADE';
  const symbolNews = newsEvents.filter((e) => e.title);
  ...
  return (
    <div className={`px-4 py-2.5 ${active ? 'bg-terminal-800/30' : ''} ${riskStatus === 'RED' && active ? 'border-l-2 border-bear-500/40' : ''}`}>
      ...
      {active ? (
        <div className="flex items-center gap-3 text-[11px] text-slate-500 mono">
          <span>E {fmtPrice(s.entry, d)}</span>
          <span className="text-bear-400/80">SL {fmtPrice(s.stopLoss, d)}</span>
          <span className="text-bull-400/80">TP {fmtPrice(s.takeProfit, d)}</span>
        </div>
      ) : (
...
function RankedRow({ s, riskStatus, newsEvents }: { s: Signal; riskStatus: RiskStatus; newsEvents: { title: string }[] }) {
  const d = decimalsFor(s.symbol);
  const hasNewsRisk = activeNewsForSymbol(s.symbol).length > 0;
  return (
    <tr className={`hover:bg-terminal-800/40 ${riskStatus === 'RED' ? 'border-l-2 border-bear-500/30' : ''}`}>
      ...
      <td className="px-3 py-2.5 text-xs text-slate-400 max-w-md">{s.reason}{hasNewsRisk && <span className="block text-[10px] text-warn-400/80 mt-1">News event nearby — reduced confidence.</span>}</td>
    </tr>
  );
}
